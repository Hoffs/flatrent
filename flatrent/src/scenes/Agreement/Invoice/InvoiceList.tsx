...
import { invoiceUrl } from "../../../utilities/Utilities";
...
class InvoiceList extends Component<IInvoiceListProps, IInvoiceListState> {
    ...
    return invoices.map((x) => (
      <FlexRow key={x.id} className={Styles.invoice}>
        <FlexColumn className={Styles.invoiceText}>
          <FlexRow className={Styles.invoiceTitle}>
            Sąskaita už laikotarpį nuo {x.invoicedPeriodFrom.split("T")[0]} iki {x.invoicedPeriodTo.split("T")[0]}
          </FlexRow>
          <FlexRow className={Styles.invoiceData}>Apmokėti iki {x.dueDate.split("T")[0]}.</FlexRow>
          <FlexRow className={Styles.invoiceStatus}>{this.getIsPaid(x)}</FlexRow>
        </FlexColumn>
  ...
  private fetchInvoices = async (_: number) => {
    try {
      const { errors, data } = await InvoiceService.getInvoices(this.props.agreement.id, this.state.invoices.length);
      if (errors !== undefined) {
